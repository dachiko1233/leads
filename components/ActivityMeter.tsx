...
export function ActivityMeter({ label = "No activity" }: { label?: string }) {
  return (
    <div
      className="inline-flex items-center gap-3 rounded-full border border-ink-line bg-ink-soft/70 px-4 py-2"
      role="img"
      aria-label={`Activity meter: ${label}`}
    >
      <span className="dead-dot inline-block h-2.5 w-2.5 rounded-full bg-danger" />
      <svg
        width="120"
        height="24"
        viewBox="0 0 120 24"
